...
from PyQt5.QtGui import QFont, QPixmap, QVector3D
    ...
    def updateGraph(self, outputDict):
        ...
        if "trackData" in outputDict:
            ...
            for trackNum, trackData in enumerate(tracks):
                ...
                # does velocity factor in sensor tilt?
                ...
                if True:
                    ...
                    self.classifierStr[trackID].setText("Track %s: buf %s"  % (trackID, str(sum(self.trackDirection[trackID]))))
        ...
        if self.camRecording and (int(round(time.time()*1000)) - self.camLastOnTimestamp > 5000):
            self.cameraStatusDisplay.setPixmap(QPixmap(notRecImgPath))
            self.camRecording = False
